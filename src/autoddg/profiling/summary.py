...
def profile_dataset(data_frame: DataFrame) -> tuple[str, str]:
    ...
    for column_meta in metadata.get("columns", []):
        ...
        if "coverage" in column_meta:
            # datamart_profiler's per-column coverage is unreliable for numeric
            # columns (it can report a lower bound of 0 for strictly-positive
            # data, etc.). Prefer pd.describe()'s min/max as the source of
            # truth whenever the column is numeric-coercible; fall back to
            # datamart only when it isn't.
            col_name = column_meta["name"]
            numeric_series: pd.Series | None = None
            if col_name in data_frame.columns:
                coerced = pd.to_numeric(data_frame[col_name], errors="coerce").dropna()
